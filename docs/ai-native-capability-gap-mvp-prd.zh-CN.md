...
| P-002 | R-008 | Codex session recovery：若 live 仍超时，记录可恢复 runId、stderr、timeout stage 和下一次续跑策略 | 部分完成 | backend | 可 | timeout fallback 已实现为 `codex_live_timeout`、`stage`、`threadId`、`sessionRecoveryHint`、`retryCommand`、stdout/stderr tail；还缺强制 timeout fixture |
...
1. 先完成 P-002 的强制 timeout fixture，确保 Codex timeout recovery 不是只靠源码静态锁。
