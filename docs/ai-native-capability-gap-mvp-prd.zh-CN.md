...
| P-017 | R-007 | 深度研究前置门：当任务依赖当前事实、联网搜索、平台/API 状态或外部能力候选时，`meta-scout` 必须先完成 source-backed research，再把 evidence handoff 给 Thinking | 未完成 | meta-scout | 可 | fixture 证明 `researchCapabilityDiscovery`、`deepResearchPlan`、source list、decisionImpactMap 在 Thinking 前存在；缺 research 时返回 `return_to_fetch` |
| P-018 | R-007/R-011 | multi-type capability inventory 质量门：能力不是 skill-only，Fetch 必须覆盖 agent、skill、script、command、MCP、runtime tool、plugin/connector、retrieval、dependency、workerTask | 未完成 | meta-artisan | 可 | 测试断言 inventory 覆盖全部类型；缺某类时记录 checked-empty 或 blocked reason，不能静默跳过 |
| P-019 | R-007 | skill 触发入口边界：`meta-theory` skill 只做 adapter，真实治理入口必须是 Warden gate，编排必须由 Conductor 生成 | 未完成 | meta-warden | 可 | 默认 run artifact 固定 `meta-theory-skill-adapter -> meta-warden-entry-gate -> meta-conductor-orchestration`；skill 不直接创建 worker 或写 canonical |
| P-020 | R-001/R-007 | 同类型多需求拆分：多个 create_skill / create_agent / workerTask-only 需求可以共享 owner，但必须保留独立 `roleInstanceId`、`shardScope`、`dependsOn` 和验收证据 | 未完成 | test | 可 | fixture 证明同类型同 owner 不折叠 worker 实例；同 repeatKey 可分组但仍保留每个需求的任务节点 |
| P-021 | R-001/R-009 | 非 agent 能力候选站点质量门：把 Genesis / Artisan / Librarian / Prism / Warden 的质量门扩展到 skill、script、MCP-provider、runtime tool 候选 | 未完成 | meta-prism | 可 | 每类候选有 boundary、loadout、least-privilege、verification、writeback policy scorecard；不合格只生成 candidate/block，不写 canonical |
...
5. 编排质量轨并行跑 P-015 / P-016 / P-017 / P-018 / P-019 / P-020 / P-021，先证明研究、能力发现、入口边界、多需求拆分和 DAG 合并规则。
6. 用户交付轨并行跑 P-012 / P-013 / P-014 / P-022 / P-023，先用数据合同和课程评分表约束 UI 与报告，而不是先做漂亮页面。
...
| Orchestration quality | P-015、P-016、P-017、P-018、P-019、P-020、P-021 | meta-conductor | P-017/P-018/P-019/P-020/P-021 产出规则与 fixture，P-016 合并成 board validator | P-016 的最终 validator 需要等其他规则产物回收后再定稿 |
