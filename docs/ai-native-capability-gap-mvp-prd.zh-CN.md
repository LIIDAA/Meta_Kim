...
| 完整产品验收 | 已测通 | `npm run meta:release:smoke`、`tests/meta-theory/30-capability-gap-complete-product.test.mjs`、`tests/meta-theory/32-meta-theory-four-product-targets.test.mjs` | 后续闭合 Codex CLI live timeout 和 Cursor live harness |
...
| 跨 runtime 投影验证 | 部分完成 | Claude / OpenClaw `meta-warden` live pass；Codex smoke pass 但 CLI live timeout；Cursor smoke pass 但 live harness unavailable | 还缺 Codex CLI live pass 和 Cursor native live-turn harness |
...
- 还不能宣称“发布级 live/native 全 runtime 完成”，因为 Codex CLI live 当前 timeout，Cursor 还没有 native live-turn harness；真实外部写回仍需要 Warden 明确批准。
...
| T-002 | Claude / Codex / Cursor / OpenClaw 四端投影验证 | 部分完成 | Claude / OpenClaw `meta-warden` live pass；Codex smoke pass + live timeout；Cursor smoke pass + live harness unavailable | 四端都有 live、smoke 或 unsupported-with-reason 证据；发布级完成还需要 Codex live pass 与 Cursor live harness |
...
|---|---|---|---|---|---|---|
| P-001 | R-008 | Codex live prompt 最小化：把 `codex --live` 输入缩短到仍能证明 Warden -> Conductor -> board -> workerTaskPackets | 未完成 | verify | 可 | `node scripts/eval-meta-agents.mjs --runtime=codex --live` 不超时且 `summary.passed` 含 `codex` |
| P-002 | R-008 | Codex session recovery：若 live 仍超时，记录可恢复 runId、stderr、timeout stage 和下一次续跑策略 | 未完成 | backend | 可 | timeout 报告包含 stage、retry command、partial artifact，不再只有进程超时 |
| P-003 | R-008 | Codex 主窗口 / 子窗口隔离复测：子窗口只跑测试与证据采集，主窗口只合并和审查 | 未完成 | test | 可 | 子窗口报告包含 threadId、命令、artifact、主窗口 review 结论 |
...
1. 先闭合 P-001 / P-002 / P-003，让 Codex live 不再只停在 smoke。
...
| R-008 跨 runtime 真实投影验证 | 部分完成 | Claude / OpenClaw 有 `meta-warden` live pass；Codex smoke pass 但 live timeout；Cursor smoke pass 且 live harness unavailable |
...
| Codex | smoke pass，live timeout | `node scripts/eval-meta-agents.mjs --runtime=codex` pass；`node scripts/eval-meta-agents.mjs --runtime=codex --live` 180s timeout | 缩短 Codex live prompt 或实现 session recovery，直到 live pass |
