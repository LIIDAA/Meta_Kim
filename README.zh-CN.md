...
5. **先定协议再开工**：task classification、任务包、交付链、审查包、验证包先说清楚，再分派
6. **review finding 要真闭环**：review finding、revision response、verification result、`closeFindings` 必须对上
7. **能并行就并行**：独立子任务不应该被无意义串行拖慢
8. **每个产出都要审查**：代码质量、安全性、架构合规、协议合规、边界越界检测
9. **每次都沉淀经验**：捕获可复用模式，记录失败，并把经验回写到 agent / skill / contract
