...
| Codex | `AGENTS.md`、`.codex/agents/`、`.agents/skills/`、`codex/config.toml.example`、`shared-skills/meta-theory.md` |
...
- Skill：Claude Code 原生支持，OpenClaw 使用可安装 skill 与 workspace skill，Codex 使用 `.agents/skills` 承接，并保留 `.codex/skills` 兼容镜像。
- MCP：通过 `scripts/mcp/meta-runtime-server.mjs` 提供统一本地服务，Claude 和 Codex 都有明确接入方式。
- Hook：Claude 有原生 hooks；OpenClaw 走原生 bundled hooks + `BOOT.md`；Codex 的审批与沙箱走宿主配置层，不伪造 1:1 仓库 hook 文件。
